...
export default function Friends() {
  ...
  useEffect(() => {
    getFriends()
      ...
      .catch(err => {
        setError('Failed to load friends debts. Invalid password or corrupted data.');
        console.error(err);
      })
  ...
  const handlePaidChange = (index, value) => {
    ...
    newFriends[index].Paid = value ? Number(value) : 0;
  ...
  const handlePaidCheckbox = (index, checked) => {
    const newFriends = [...friends];
    if (checked) {
      newFriends[index].Paid = newFriends[index].Amt;
    } else {
      newFriends[index].Paid = 0;
    }
    setFriends(newFriends);
    setEditing(prev => ({ ...prev, [index]: true }));
  };
  ...
  const handleSave = async () => {
    try {
      ...
      setError('Failed to save friends data.');
      console.error(err);
    }
  };

  const getBoderColor = (index) => {
    const debt = friends[index];
  ...
  const totalAmt = friends.reduce((sum, debt) => sum + (Number(debt.Amt) || 0), 0);
  const totalPaid = friends.reduce((sum, debt) => sum + (Number(debt.Paid) || 0), 0);
  ...
  return (
    <div style={{ width: '100%', display: 'flex', justifyContent: 'center' }}>
      <div style={{ maxWidth: 360, width: '100%', padding: '20px' }}>
        ...
        {loading && <p>Loading...</p>}
        {!loading && friends.length === 0 && <p>No friends debts</p>}
        {!loading && friends.length > 0 && (
          <ul style={{ listStyle: 'none', padding: 0 }}>
            {friends.map((debt, idx) => {
              const paid = Number(debt.Paid) || 0;
